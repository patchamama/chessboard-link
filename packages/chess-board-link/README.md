...
| **ChessUp** | ✅ Moves + LEDs over **BLE (Nordic UART)** — **verified live on hardware** | UUIDs `6e40000x`, **raw bytes (no parity)**, handshake `64` → `66 len …FEN` → `b9 …`, move inbound `163` `[163,53,fromCol,fromRow,toCol,toRow]`, poll `21`, ACK `33`, **`setLeds`** (8-byte bitmap) |
